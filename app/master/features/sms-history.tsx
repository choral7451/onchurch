...
import { ApiError, onchurchMaster, type SmsLog } from "@/lib/api-client";
...
export function SmsHistoryFeature({ reloadKey = 0 }: { reloadKey?: number }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await onchurchMaster.listSmsLogs({ keyword: query, page: 1, size: PAGE_SIZE });
  ...
  const loadMore = useCallback(async () => {
    ...
    setLoadingMore(true);
    try {
      const res = await onchurchMaster.listSmsLogs({ keyword: query, page: next, size: PAGE_SIZE });
  ...
  return (
    <div>
      <h2 className="text-xl font-bold text-gray-900">문자 발송 내역</h2>
      <p className="mt-1 text-sm text-gray-500">누구에게 어떤 내용의 문자를 보냈는지 확인할 수 있습니다.</p>

      <input
        type="text"
        value={keyword}
        onChange={(e) => setKeyword(e.target.value)}
        placeholder="휴대폰 번호, 제목, 내용 검색"
