...
export function PrayerForm({ slug, categories, scopes }: Props) {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    if (!anon && !name.trim()) { setErrMsg("이름을 입력해주세요. 익명으로 보내려면 아래 체크박스를 선택해주세요."); return; }
    if (!content.trim()) { setErrMsg("기도 제목 본문을 입력해주세요."); return; }
    if (!consent) { setErrMsg("개인정보 수집 및 이용에 동의해주세요."); return; }
    ...
    try {
      ...
      setErrMsg(err instanceof ApiError ? err.message : "전송 중 오류가 발생했습니다. 잠시 후 다시 시도해주세요.");
    } finally {
      setSubmitting(false);
    }
  }
  ...
  if (submitted) {
    return (
      <div className="card" style={{ textAlign: "center", padding: "64px 32px" }}>
        ...
        <button type="button" className="btn btn-primary" onClick={() => setSubmitted(false)}>또 다른 기도 요청하기</button>
      </div>
    );
  }
  ...
  return (
    <form className="card" style={{ padding: 40 }} onSubmit={onSubmit}>
      ...
      <div className="form-grid">
        <div className="form-row">
          ...
          <input
            ...
            placeholder={anon ? "익명으로 진행됩니다" : "성함을 입력해주세요"}
            disabled={anon}
            required={!anon}
          />
        </div>
        <div className="form-row">
          ...
          <input
            ...
            placeholder="응답 받기를 원하시면 입력해주세요"
          />
        </div>
        <div className="form-row">
        ...
        <div className="form-row full">
          ...
          <textarea
            ...
            placeholder="마음에 있는 것을 자유롭게 적어주세요. 짧아도 괜찮습니다."
            required
          />
        </div>
        <div className="form-row full">
      ...
      <div style={{ display: "flex", justifyContent: "flex-end", gap: 12, marginTop: 32, paddingTop: 24, borderTop: "1px solid var(--line)" }}>
        <button type="button" className="btn btn-secondary" onClick={reset} disabled={submitting}>초기화</button>
        <button type="submit" className="btn btn-primary" disabled={submitting}>
          <Icon.pray style={{ width: 14, height: 14 }} />
          {submitting ? "보내는 중..." : "기도 요청 보내기"}
        </button>
      </div>
    </form>
  );
